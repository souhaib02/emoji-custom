...
import {
  ...
  EmojiData,
  EmojiEvent,
} from 'ngx-emoji-custom/ngx-emoji';
...
export class PickerComponent implements OnInit, OnDestroy {
  ...
  CUSTOM_CATEGORIES: EmojiCategory[] = [];
  ...
  ngOnInit() {
    ...
    if (this.custom.length > 0) {
      this.CUSTOM_CATEGORIES = this.custom.map(emoji => ({
          ...emoji,
          // `<Category />` expects emoji to have an `id`.
          //id: emoji.shortNames[0],
           emojis: emoji.emojis?.map(e => ({...e, custom: true, }))!
        })
      );

      allCategories.push(...this.CUSTOM_CATEGORIES);
    }
    this.allCustomEmojis = this.CUSTOM_CATEGORIES.flatMap(e=>e.emojis);
  ...
  handleEmojiOver($event: EmojiEvent) {
    ...
    const emojiData = this.CUSTOM_CATEGORIES.flatMap(x => x.emojis).find(
